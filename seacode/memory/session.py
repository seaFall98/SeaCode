"""会话持久化：JSONL 增量追加、元数据分离、compact_boundary 内联重建与 resume。"""
...
# 默认保留期：30 天未活跃会话由 cleanup 删除。
DEFAULT_MAX_AGE_DAYS = 30
# 会话标题取首条 user 消息前 50 字符。
TITLE_MAX_LENGTH = 50

# 会话摘要生成使用的系统提示词；裸 LLM 调用，不带工具。
...
class RecordType(StrEnum):
    """JSONL 记录类型；COMPACT_BOUNDARY 是 Layer-2 压缩后的内联重建标记。"""
    ...
    # Layer-2 compact 标记：内联摘要 + 原样保留的 keep 尾部，
    # resume 仅凭此标记即可重建压缩后的状态，无需重放标记之前的原始前缀。
    COMPACT_BOUNDARY = "compact_boundary"


@dataclass
class SessionRecord:
    """一条 JSONL 记录；content 依 type 不同承载字符串或 content-blocks 列表。"""
    ...
    # 将一条 Message 拆为若干 SessionRecord：
    # - tool_results 独立成条（按 tool_use_id 关联回 assistant 工具调用）
    # - assistant 的 tool_uses 转为 content-blocks 列表
    # - 普通 user/assistant 文本成单条
    @classmethod
...
def records_to_messages(records: list[SessionRecord]) -> list[Message]:
    ...
    通过 tool_use_id 重新配对 assistant 的 tool_uses 与紧随其后的 tool_results。
    遇 COMPACT_BOUNDARY 内联展开为摘要 user 消息 + keep 尾部。
    """
    ...
    for record in records:
        ...
        if record.type == RecordType.COMPRESSION:
            messages.append(
                Message(
                    role="user",
                    content="本次会话延续自之前的对话，因上下文空间不足进行了压缩。"
                    "以下是早期对话的摘要：\n\n" + (record.content or ""),
                )
            )
            continue
...
def validate_message_chain(records: list[SessionRecord]) -> int:
    """返回最后所有 tool_use 都有对应 tool_result 的位置（截断点）。

    孤立的 tool_use（无配对 tool_result）通常由坏行或中断写入导致，
    resume 时截断到最后一个完整位置，避免 LLM 因缺少 tool_result 报错。
    """
    last_valid = 0
...
@dataclass
class SessionMeta:
    """会话元数据；与 .meta JSON 文件一一对应，list() 只扫 .meta 不开 JSONL。"""
    ...
    # 写入 .meta JSON 文件，使用 ensure_ascii=False 保留中文标题。
    ...
    # 从 .meta 文件加载；损坏返回 None 让 list() 跳过。
    @classmethod
...
class Session:
    """活跃会话句柄；持有 JSONL 文件句柄，append 时 flush 保证崩溃安全。"""
    ...
    # 追加一条对话消息：先写文件再更新 meta，崩溃时可从文件重建。
...
# 会话摘要生成：取最近 10 条消息裸 LLM 调用一句话摘要；失败返回空字符串。
async def generate_session_summary(
    client: Any, conversation: ConversationManager, protocol: str
) -> str:
    del protocol  # 仅作语义占位；裸调用 client.stream 不依赖 protocol。
    ...
    # 裸 LLM 调用：传入消息序列（不带工具），system 用摘要提示词约束输出。
    # LLMClient.stream 期望 Sequence[Message]，因此传 summary_conv.history 而非管理器本身。
    collected = ""
    try:
        async for event in client.stream(
            summary_conv.history, system=SESSION_SUMMARY_PROMPT
        ):
...
# 生成 session ID：session_YYYYMMDD_HHMMSS_xxxx（4 位字母数字后缀）。
...
class SessionManager:
    """会话目录管理者：create/list/resume/delete/cleanup。"""

    def __init__(self, work_dir: str) -> None:
        self._sessions_dir = Path(work_dir) / SESSIONS_DIR
        self._sessions_dir.mkdir(parents=True, exist_ok=True)

    # 创建新会话：生成 ID + .jsonl + .meta，文件以追加模式打开。
    ...
    # 恢复历史会话：逐行解析 JSONL 跳过坏行、定位最后 boundary、校验链配对、
    # 重建消息；以追加模式重开文件句柄，后续 append 直接续写。
    ...
    # 清理 max_age_days 天未活跃的会话；返回删除条数。
