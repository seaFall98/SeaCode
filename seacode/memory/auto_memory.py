"""自动记忆管理器：独立 .md 文件 + frontmatter + MEMORY.md 索引。"""
...
# 四种记忆类型：user/feedback → 用户级；project/reference → 项目级。
...
# MEMORY.md 截断限制：行数与字节双上限。
MAX_ENTRYPOINT_LINES = 200
MAX_ENTRYPOINT_BYTES = 25_000

# frontmatter 正则；YAML-ish 头部，--- 围栏。
...
# 返回项目级记忆目录路径：<projectRoot>/.seacode/memory/。
# 保留尾部分隔符，确保前缀匹配不会误命中类似 memoryxyz 的路径。
# 支持 SEA_REMOTE_MEMORY_DIR 环境变量覆盖（远程开发场景）。
def get_auto_mem_path(project_root: str) -> str:
    override = os.environ.get("SEA_REMOTE_MEMORY_DIR", "")
    if override:
        return override.rstrip(os.sep) + os.sep
    abs_root = os.path.abspath(project_root)
    return os.path.join(abs_root, ".seacode", "memory") + os.sep


# 返回用户级记忆目录路径：~/.seacode/memory/。
...
def get_user_auto_mem_path() -> str:
    ...
    if not home:
        return ""
    return os.path.join(home, ".seacode", "memory") + os.sep
...
# 构建记忆系统提示：行为指令 + 两级 MEMORY.md 索引内容。
...
def _build_entrypoint_section(scope_label: str, entrypoint_path: str) -> str:
    ...
    return (
        header
        + f"\nThis {ENTRYPOINT_NAME} is currently empty. When you save new {scope_label.lower()}-level memories, "
        "add their pointers here."
    )
...
class MemoryManager:
    ...
    提供系统提示构建、裸 LLM 提取、记忆列表展示与清理能力。
    实际的写入通过 agent 的 Write 工具完成，此类负责协调目录与索引。
    """

    def __init__(self, project_root: str) -> None:
        abs_root = os.path.abspath(project_root)
        self._project_root = abs_root
        # 用户级：~/.seacode/memory/ — user/feedback 类型记忆
        self._user_mem_dir = get_user_auto_mem_path()
        # 项目级：<projectRoot>/.seacode/memory/ — project/reference 类型记忆
        self._mem_dir = get_auto_mem_path(abs_root)
        # 上次提取推进到的 history 长度，避免重复处理同一段对话。
    ...
    def user_path(self) -> Path:
        ...
        return Path.home() / ".seacode" / "memory" / ENTRYPOINT_NAME
    ...
    def user_mem_dir(self) -> Path:
        """用户级记忆目录（~/.seacode/memory/）。"""
        if self._user_mem_dir:
            return Path(self._user_mem_dir.rstrip(os.sep))
        return Path.home() / ".seacode" / "memory"

    @property
    def project_mem_dir(self) -> Path:
        """项目级记忆目录（<project>/.seacode/memory/）。"""
    ...
    # 扫描已有记忆文件，生成 manifest 给 LLM 做去重提示。
    ...
    # 触发记忆提取：裸 LLM 流式调用 + 结构化输出解析 + 文件写入 + 索引更新。
    # 发送已有记忆 manifest 做去重提示；NONE 或无 MEMORY_NAME 跳过。
    async def extract(
        self,
        client: Any,
        ...
        # 裸 LLM 调用：传入消息序列（不带工具），system 用提取提示词约束输出格式。
        # LLMClient.stream 期望 Sequence[Message]，因此传 extract_conv.history 而非管理器本身。
        collected = ""
        try:
            async for event in client.stream(
                extract_conv.history, system="You are a memory extraction assistant."
            ):
        ...
        for block in blocks:
            ...
            # 更新 MEMORY.md 索引：检查 {name}.md 是否已存在避免重复添加索引行。
