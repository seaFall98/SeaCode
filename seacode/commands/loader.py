"""从 .seacode/commands/ 目录加载自定义 Markdown 命令。
...
def _make_prompt_handler(body: str):
    ...
    追加 "## User Request" 段落，让模型看到用户的补充输入。
    """
...
def _parse_command_file(base_dir: Path, path: Path) -> Command | None:
    """解析单个 .md 命令文件：提取命令名、frontmatter 元信息与正文。"""
    try:
...
def load_user_commands(work_dir: str) -> list[Command]:
    ...
    搜索路径（后者覆盖前者）：
      1. ~/.seacode/commands/
      2. <work_dir>/.seacode/commands/
    """
    dirs: list[str] = []
    home = Path.home()
    dirs.append(str(home / ".seacode" / "commands"))
    dirs.append(str(Path(work_dir) / ".seacode" / "commands"))
...
def register_user_commands(registry: CommandRegistry, work_dir: str) -> int:
    """加载用户自定义命令并注册到注册中心；与内置命令同名时跳过。

    返回成功注册的命令数量；同名或别名冲突记 debug 日志后跳过，
    不影响其它命令与启动流程。
    """
    commands = load_user_commands(work_dir)
    registered = 0
    for cmd in commands:
        try:
            ...
            # 与内置命令或已注册命令同名/别名冲突时跳过，内置命令优先。
            log.debug("skip user command %s (name/alias conflict)", cmd.name)
    return registered
