"""Textual driver：跳过 alternate screen，让 TUI 输出保留在主终端 scrollback。"""
...
class NoAltScreenDriver(_BaseDriver):
    """跳过备用屏（alternate screen）的 driver。

    让输出保留在主终端的滚动回看（scrollback）区域中，方便用户向上翻阅
    历史交互记录。自动根据平台选择 LinuxDriver 或 WindowsDriver 作为基类。

    原理：去掉 alt screen 切换码（\x1b[?1049h / \x1b[?1049l），并在进入
    应用模式时输出足够多的空行，将已有终端内容推入 scrollback，
    Textual 随后在"新页面"上渲染。
    """
    ...
    def write(self, data: str) -> None:
        # 过滤备用屏切换序列，使渲染留在主屏 scrollback 中。
